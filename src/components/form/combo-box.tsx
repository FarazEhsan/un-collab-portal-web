...
const ComboBox = ({multiple, people}:any) => {
    ...
    return (
        <Combobox as="div" value={selectedPeople} onChange={setSelectedPeople}
                  multiple={multiple}>
            <Combobox.Label
                className="block text-sm font-medium leading-6 text-gray-900 dark:text-gray-100">Assigned
                to</Combobox.Label>
            {multiple && selectedPeople?.length > 0 && (
                <div className="grid grid-cols-3 gap-2">
                    {selectedPeople?.map((person:any) => (
                        <Badge key={person.id} text={person.name}/>
                    ))}
                </div>
            )}
