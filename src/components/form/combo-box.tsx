...
const ComboBox = ({multiple, items, label, selectedData, setSelectedData}:any) => {
        ...
        <Combobox as="div" value={selectedData} onChange={setSelectedData}
            ...
            {multiple && selectedData?.length > 0 && (
                <div className="grid grid-cols-3 gap-2">
                    {selectedData?.map((item:any) => (
                        <Badge key={item.id} text={items.filter((d:any) => (d.id === item))[0]?.name}/>
                    ))}
                </div>
            )}
            <div className="relative mt-2">
                <Combobox.Input
                    ...
                    displayValue={(item) => items.filter((d:any) => (d.id === item))[0]?.name}
                />
